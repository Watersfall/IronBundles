...
public class CustomBundleItem extends Item
{
	...
	public boolean onStackClicked(ItemStack stack, Slot slot, ClickType clickType, PlayerEntity player)
	{
		...
		else
		{
			...
			{
				getTopStack(stack).ifPresent((itemStack2) -> addToBundle(stack, slot.insertStack(itemStack2)));
			}
	...
	private int addToBundle(ItemStack bundle, ItemStack stack)
	{
		...
		{
			NbtCompound tag = bundle.getOrCreateTag();
	...
	private static Stream<ItemStack> getBundledStacks(ItemStack stack)
	{
		...
		else
		{
			...
			Stream<NbtElement> stream = listTag.stream();
	...
	private class BundleInventory implements Inventory
	{
		...
		private BundleInventory(ItemStack bundle, NbtList tag)
		{
			this.bundle = bundle;
			this.readTags(tag);
		}
